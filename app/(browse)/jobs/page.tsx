  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        {user && (
          <div className="text-right">
            ...
            <p className="text-sm font-semibold text-ink-800 mt-0.5">
              {makerLevel === 0 ? 'Community Maker' : makerLevel === 1 ? 'Verified Quality' : makerLevel === 2 ? 'Engineering Certified' : 'Production Partner'}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Link href="/jobs" className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors ${!job_type ? 'bg-ink-900 text-white border-ink-900' : 'border-warm-300 text-warm-600 hover:border-ink-400'}`}>
          All types
        </Link>
        {JOB_TYPES.map((jt) => {
          const locked = user ? jt.minCertLevel > makerLevel : false
          const active = job_type === jt.value
          const params = new URLSearchParams({ ...(material && { material }), ...(q && { q }), ...(shipping && { shipping }), job_type: jt.value })
          return (
            <Link key={jt.value} href={`/jobs?${params}`}
              className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors flex items-center gap-1 ${
                active ? 'bg-ink-900 text-white border-ink-900'
                : locked ? 'border-warm-200 text-warm-300 cursor-default'
      ...
      <form className="card p-3 flex flex-wrap gap-2 items-center">
        ...
        <button type="submit" className="rounded-xl bg-ink-800 px-4 py-2 text-sm font-medium text-white hover:bg-ink-900 transition-colors">Filter</button>
        {hasFilters && <Link href="/jobs" className="rounded-xl border border-warm-300 px-4 py-2 text-sm text-warm-600 hover:bg-warm-100 transition-colors">Clear</Link>}
      </form>
