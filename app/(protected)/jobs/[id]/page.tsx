...
export default async function JobDetailPage({ params, searchParams }: PageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {canSeeFiles && files && files.filter((f) => f.file_type !== 'pre_ship_photo' && f.file_type !== 'delivery_photo').length > 0 && (
        <div className="card p-6">
          ...
          <ul className="space-y-2">
            {files.filter((f) => f.file_type !== 'pre_ship_photo' && f.file_type !== 'delivery_photo').map((f) => (
              <li key={f.id} className="flex items-center justify-between rounded-xl bg-warm-50 px-3 py-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {f.file_type !== 'link' && f.file_size ? <span className="text-xs text-warm-400 flex-shrink-0">{formatFileSize(f.file_size)}</span> : null}
                </div>
                <a href={f.file_url} target="_blank" rel="noopener noreferrer"
                  className="text-sm text-ink-600 hover:text-ink-700 font-medium ml-2 flex-shrink-0">
                  {f.file_type === 'link' ? 'Open link ↗' : 'Download'}
                </a>
              </li>
            ))}
          </ul>
      ...
      {/* Delivery photos (uploaded by customer on confirmation) */}
      {canSeeFiles && files && files.filter((f) => f.file_type === 'delivery_photo').length > 0 && (
        <div className="card p-6">
          <div className="flex items-center gap-2 mb-1">
            <h2 className="font-semibold text-warm-900">Delivery photos</h2>
            <span className="rounded-full bg-blue-100 text-blue-700 text-xs font-medium px-2 py-0.5">Confirmed by customer</span>
          </div>
          <p className="text-xs text-warm-400 mb-4">Uploaded by the customer when confirming receipt.</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {files.filter((f) => f.file_type === 'delivery_photo').map((f) => (
              <a key={f.id} href={f.file_url} target="_blank" rel="noopener noreferrer"
                className="group relative aspect-square rounded-xl overflow-hidden border border-warm-200 hover:border-ink-300 transition-colors">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={f.file_url} alt="Delivery photo" className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                  <span className="opacity-0 group-hover:opacity-100 text-white text-xs font-medium transition-opacity">View full size ↗</span>
                </div>
              </a>
